...
async def qa_bot(request: Request, _schema: Parms):
    try:
        ...
        document_ids = body.get("document_ids")  # Expecting a comma-separated string of document IDs
        ...
        async def generate_response():
            ...
            if query_intent == "unstructured":
                ...
                k=3,
                filter={"id": doc_id}  # Assuming 'id' is the correct key
            )
        )
                else:
                ...
                async for chunk in chat_model.astream(prompt_template.format(history=memory.load_memory_variables({}), input=refined_query_text)):
                      ...
                      if content_chunk.strip():
                        ...
                        while '\n' in buffer:
                            ...
                            async for part in stream_chunk(line):
                                yield part
                                print (line)
                ...
                if buffer:
                    print(buffer)
                    async for part in stream_chunk(buffer):
                        yield part
